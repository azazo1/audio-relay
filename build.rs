...
fn resolve_manual_opus_dir(
    ...
    if matches!(preference, OpusLinkPreference::PreferStatic) && !link_statically {
        println!(
            "cargo:warning=audio-relay found Opus in {} but could not confirm a static library; falling back to the default linker mode",
            dir.display()
        );
    }
...
fn link_opus_windows(target: &str, preference: OpusLinkPreference) {
    ...
    println!(
        "cargo:warning=audio-relay could not locate an Opus library automatically; set OPUS_LIB_DIR/OPUS_LIB_NAME or VCPKG_ROOT if link fails"
    );
}
...
fn link_opus_pkg_config(preference: OpusLinkPreference) {
    ...
    if try_link_opus_pkg_config(false) {
        if matches!(preference, OpusLinkPreference::PreferStatic) {
            println!(
                "cargo:warning=audio-relay could not obtain static pkg-config flags for opus; falling back to the default linker mode"
            );
        }
        return;
    }
...
fn try_link_opus_pkg_config(link_opus_statically: bool) -> bool {
    ...
    let output = match command.args(["--libs", "--cflags", "opus"]).output() {
        ...
        Err(err) => {
            if link_opus_statically {
                println!(
                    "cargo:warning=audio-relay failed to invoke pkg-config --static for opus ({err}); retrying without --static"
                );
                return false;
            }
